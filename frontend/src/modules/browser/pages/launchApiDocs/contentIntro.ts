...
\`\`\`text
内核管理 -> 准备 fingerprint-chromium 内核
代理池配置 -> 新建代理
插件包管理 -> 安装 / 导入插件
...
\`\`\`text
1. 内核管理：准备 fingerprint-chromium 内核
2. 代理池配置：导入或录入代理
3. 插件包管理：安装插件，按需限制实例
...
内核可安装到任意目录，在「内核管理」中通过「选择目录」注册即可。

## 准备方式

\`\`\`text
1. 内核管理 -> 前往 Releases 页面获取链接 -> 下载并解压到任意目录
2. 内核管理 -> 新增内核 -> 选择目录 -> 保存
\`\`\`
