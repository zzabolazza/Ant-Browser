...
Facade 当前推荐配套使用的浏览器内核，来源于开源项目 [fingerprint-chromium](https://github.com/adryfish/fingerprint-chromium)。

如果你正在寻找可直接下载和维护的指纹内核版本，建议先查看它的 Releases 页面：

- <https://github.com/adryfish/fingerprint-chromium/releases>

这个项目为 Facade 的内核准备提供了直接可用的基础来源，这里先对原项目做明确推荐与致谢。

Facade 的目标很明确：在一台桌面设备上，帮助用户稳定管理多个彼此隔离的浏览器实例，并配合代理池、浏览器内核和快捷启动能力完成日常运营或测试工作。
...
- 增强代理池：优化原生代理链接的导入、编辑和预览能力，支持 HTTP(S) / SOCKS5 / 直连节点批量导入
- 优化代理检测：新增测速目标、IP 健康检测目标和检测超时配置
...
- 补齐 macOS unsigned 内测构建链路：支持在原生 macOS 主机上打包 `.app` / `.zip`，并将用户状态目录放到 `~/Library/Application Support/facade`
...
5. macOS unsigned 包解压后运行 `Facade-<version>-macos-<arch>.app`；如被 Gatekeeper 拦截，请对本机测试包执行 `xattr -dr com.apple.quarantine <app路径>` 后再打开
...
```bash
go run ./backend/cmd/profile-recover --app-root '/path/to/Facade'
go run ./backend/cmd/profile-recover --app-root '/path/to/Facade' --apply
```
